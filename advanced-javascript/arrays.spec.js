...
describe('Arrays', () => {
  ...
  describe('map', () => {
    ...
    it("should add 2 to each element", () => {
      expect(arrayFunctions.map(([1,2,3]), (ele) => ele + 2)).toEqual(
        [3,4,5]);
        expect(arrayFunctions.map(([100,200,300]), (ele) => ele + 2))
        .toEqual([102,202,302]); 
    })
    it('should multiply each number by 3', () => {
      expect(arrayFunctions.map(([1,2,3]), (num) => num * 3)).toEqual
      ([3,6,9]);
    })
  });
});
...
describe('filter', () => {
  it('should return all the even numbers', () => {
    expect(arrayFunctions.filter([1,2,4,5,6], (num) => {
      return num % 2 === 0;   
    })).toEqual([2,4,6])
  })
...
//flatten
describe('flatten', () => {
  it('should return a single array all elements but if they are arrays then concat those arrayss', () => {
