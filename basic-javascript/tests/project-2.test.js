...
describe("PROJECT 2", () => {
  ...
  describe("greeting", () => {
    ...
    it("should return a greeting in the language chosen as an argument", () => {
      assert.equal(greeting("German"), "Guten Tag!");
      assert.equal(greeting("Spanish"), "Hola!");
      assert.equal(greeting("japanese"), "Hello!");
    });
  });
  describe("getBiggest", () => {
    const getBiggest = funcs.getBiggest;
    it("should return the larger of two numbers", () => {
      assert.equal(getBiggest(3, 99), 99);
      assert.typeOf(getBiggest(3, 88), "number");
    });
  });
  describe("isTenOrFive", () => {
    ...
    it("should return true if the number is either 10 or 5", () => {
      ...
      assert.typeOf(isTenOrFive(9), "boolean");
    });
  });
  describe("isInRange", () => {
    const isInRange = funcs.isInRange;
    it("should return a boolean if a number falls between the range of 20 and 50", () => {
      assert.equal(isInRange(70), false);
      assert.equal(isInRange(35), true);
      assert.typeOf(isInRange(29), "boolean");
    });
  });
  describe("isInteger", () => {
  ...
  describe("wordsToSentence", () => {
    ...
    it('should give result of wordsToSentence to be "Hello Jason Man"', () => {
      expect(wordsToSentence(["Hello", "Jason", "Man"])).to.equal(
        "Hello Jason Man"
      );
    });
  });
